"""Smoke test AR-fill mode (SGLANG_DLLM_AR_FILL=1).

Compare 3 configs on 3 samples:
  A: legacy bs=32 no SA (baseline)
  B: bs=32 + SA + newline pad (current explanation-focused)
  C: bs=32 + SA + newline pad + AR-fill (causal within chunk, L→R commit)
"""
...
def run_phase(label, section_align, out_json):
    ...
    print(f"[{label}] AR_FILL={os.environ.get('SGLANG_DLLM_AR_FILL','-')} SA={section_align}", flush=True)
    bundle = loader.load(algorithm="mdm", engine_block_size=32)
    gen_kwargs = dict(temperature=0.0, block_size=32, section_align=section_align)
...
def main():
    if len(sys.argv) > 1 and sys.argv[1] == "--phase":
        run_phase(sys.argv[2], sys.argv[3] == "true", sys.argv[4])
        return

    configs = [
        ("legacy_no_SA",   "false", {}, "/tmp/ar_A.json"),
        ("SA_newline",     "true",  {}, "/tmp/ar_B.json"),
        ("SA_AR_fill",     "true",  {"SGLANG_DLLM_AR_FILL": "1"}, "/tmp/ar_C.json"),
    ]
    for label, sa, env_set, out_json in configs:
        env = dict(os.environ)
        env.pop("SGLANG_DLLM_AR_FILL", None)
        env.pop("SGLANG_DLLM_THRESHOLD", None)
        env.update(env_set)
        env["DLLM_FWD_LOG"] = "1"
        print(f"\n========== {label} ==========", flush=True)
        rc = subprocess.call(
            [sys.executable, "-u", __file__, "--phase", label, sa, out_json], env=env,
        )
        if rc != 0:
            print(f"{label} failed (rc={rc})"); continue

    print("\n========== SUMMARY ==========", flush=True)
    print(f"{'config':<16} | {'mean_lat':>9} | {'mean_ADE':>9} | per-sample ADE", flush=True)
    for label, _, _, out_json in configs:
        if not os.path.exists(out_json): continue
        rs = json.load(open(out_json))
        ...
        print(f"{label:<16} | {mean_lat:>8.2f}s | {mean_ade:>8.2f}m | {per}", flush=True)
