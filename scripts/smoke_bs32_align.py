"""bs=32 + section_align (vs legacy bs=32 no_align).

Tests whether section-aligned chunking at small block size preserves
explanation quality while still cleaning section boundaries.
"""
...
def run_phase(label, section_align, out_json):
    ...
    print(f"[{label}] loading engine bs=32, section_align={section_align}...", flush=True)
...
def main():
    ...
    configs = [
        ("legacy_bs32_noSA", "false", "/tmp/b32_legacy.json"),
        ("bs32_SA",          "true",  "/tmp/b32_SA.json"),
    ]

    for label, sa, out_json in configs:
        env = dict(os.environ)
        print(f"\n========== {label} ==========", flush=True)
        rc = subprocess.call(
            [sys.executable, "-u", __file__, "--phase", label, sa, out_json],
            env=env,
        )
        if rc != 0:
    ...
    print("\n========== SUMMARY ==========", flush=True)
    print(f"{'config':<22} | {'mean_lat':>9} | {'mean_ADE':>9} | per-sample ADE", flush=True)
    for label, _, out_json in configs:
        ...
        print(f"{label:<22} | {mean_lat:>8.2f}s | {mean_ade:>8.2f}m | {per}", flush=True)
