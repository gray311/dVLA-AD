"""Sweep steps_per_chunk on bs=32 + SA + newline pad."""
...
def run_phase(label, steps, out_json):
    ...
    print(f"[{label}] steps_per_chunk={steps}", flush=True)
    bundle = loader.load(algorithm="mdm", engine_block_size=32)
    gen_kwargs = dict(temperature=0.0, block_size=32, section_align=True, steps_per_chunk=steps)
...
def main():
    ...
    for steps in [4, 8, 12, 16]:
        ...
        env["DLLM_FWD_LOG"] = "1"
        out_json = f"/tmp/steps{steps}.json"
        print(f"\n========== steps={steps} ==========", flush=True)
        rc = subprocess.call(
            [sys.executable, "-u", __file__, "--phase", f"s{steps}", str(steps), out_json], env=env,
        )
        if rc != 0:
            print(f"steps={steps} failed (rc={rc})"); continue

    print("\n========== SUMMARY ==========", flush=True)
    print(f"{'config':<10} | {'mean_lat':>9} | {'mean_ADE':>9} | per-sample ADE", flush=True)
    for steps in [4, 8, 12, 16]:
        path = f"/tmp/steps{steps}.json"
        ...
        print(f"steps={steps:<5} | {mean_lat:>8.2f}s | {mean_ade:>8.2f}m | {per}", flush=True)
