...
class Req:
    ...
    def _dllm_needs_prompt_prefill(self):
        """Check if this dLLM request needs a separate prompt prefill phase."""
        return (
            self.multimodal_inputs is not None
            and hasattr(self.multimodal_inputs, 'mm_items')
            and self.multimodal_inputs.mm_items
        )
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def prepare_for_extend(self):
        ...
        if self.is_dllm():
            # Check if any request is still in prompt prefill phase
            all_prefilled = all(
                getattr(r, '_dllm_prompt_prefilled', True) for r in self.reqs
            )
            if all_prefilled:
    ...
    def get_model_worker_batch(
        ...
        return ModelWorkerBatch(
            ...
            dllm_template_rep_penalty_positions=[
                getattr(req.sampling_params, "dllm_template_rep_penalty_positions", None)
                for req in self.reqs
            ],
            dllm_template_rep_penalty=[
                getattr(req.sampling_params, "dllm_template_rep_penalty", 0.0)
                for req in self.reqs
            ],
            dllm_template_steps_per_chunk=[
                getattr(req.sampling_params, "dllm_template_steps_per_chunk", 4)
                for req in self.reqs
            ],
            dllm_template_explanation_l2r=[
                getattr(req.sampling_params, "dllm_template_explanation_l2r", False)
                for req in self.reqs
            ],
...
class ModelWorkerBatch:
    ...
    dllm_template_explanation_l2r: Optional[List[bool]] = None
    dllm_config: Optional[DllmConfig] = None
