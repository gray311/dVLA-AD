...
class SchedulerOutputProcessorMixin:
    ...
    def process_batch_result_dllm_prefill(
        self: Scheduler,
        batch: ScheduleBatch,
        result: GenerationBatchResult,
    ):
        """Handle the result of a dLLM prompt prefill (EXTEND mode).

        Captures the AR first token from prefill logits and keeps the
        request as chunked so the next round adds mask tokens and
        switches to DLLM_EXTEND for block decoding.
        """
        if result.copy_done is not None:
            result.copy_done.synchronize()
        ...
        assert len(batch.reqs) == 1, "dLLM batch size is currently expected to be 1"
        req = batch.reqs[0]

        # Record prefill completion time (same as process_batch_result_prefill)
        if req.time_stats.prefill_finished_ts == 0.0:
            req.time_stats.prefill_finished_ts = time.time()

        if result.next_token_ids is not None:
            ar_token = result.next_token_ids[0]
    ...
    def process_batch_result_dllm(
        ...
        if not req.finished():
            # Speculative partial acceptance: dllm_ids may be longer than
            # the actual KV entries.  Truncate so prefix_indices stays in sync.
            # Also free the KV slots for rejected positions (doing it here
            # instead of in the algorithm avoids double-free when the request
            # finishes in the same block via release_kv_cache).
            if hasattr(req, "dllm_ids"):
                expected_len = len(req.origin_input_ids) + len(req.output_ids)
                if len(req.dllm_ids) > expected_len:
                    # Free rejected KV slots
                    rejected_kv_locs = self.req_to_token_pool.req_to_token[
                        req.req_pool_idx, expected_len : len(req.dllm_ids)
                    ]
                    self.token_to_kv_pool_allocator.free(rejected_kv_locs)

                    req.dllm_ids = req.dllm_ids[:expected_len]
                    req.fill_ids = req.dllm_ids
                    req.kv_committed_len = expected_len
                    req.kv_allocated_len = expected_len
